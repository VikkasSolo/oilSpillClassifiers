...
# # Allowlist the Discriminator class
# add_safe_globals([Discriminator])
...
D = torch.load(pthFile, weights_only=False)
...
def testClassify(dataloader):
    ...
    for j in range(predlabels.size):
        ...
        if predlabels[j]==gt[j] and gt[j]==1 :
            ...
            # display image
            cv2.imshow("Detected: Oil Spill", image)
            cv2.waitKey(0)
            cv2.destroyAllWindows()
        else:
            print(img_path[j],j,'no oil spill')
            image = cv2.imread(img_path[j])
            # display image
            cv2.imshow("Detected: No Oil Spill", image)
            cv2.waitKey(0)
            cv2.destroyAllWindows()
